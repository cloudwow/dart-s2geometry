...
class S1Interval {
   ...
   */
  static S1Interval copy(S1Interval other) {
    return S1Interval(lo: other.lo, hi: other.hi);
  }
   ...
   */
  S1Interval({double lo, double hi, bool checked: false}) {
    double newLo = lo;
    double newHi = hi;
    if (!checked) {
      if (lo == -PI && hi != PI) {
  ...
  static S1Interval empty() {
    return new S1Interval(lo: PI, hi: -PI, checked: true);
  }
  ...
  static S1Interval fromPoint(double p) {
    ...
    return new S1Interval(lo: p, hi: p, checked: true);
  }
  ...
  static S1Interval fromPointPair(double p1, double p2) {
    // assert (Math.abs(p1) <= PI && Math.abs(p2) <= PI);
    if (p1 == -PI) {
      p1 = PI;
    }
    if (p2 == -PI) {
    ...
    if (positiveDistance(p1, p2) <= PI) {
      return new S1Interval(lo: p1, hi: p2, checked: true);
    } else {
      return new S1Interval(lo: p2, hi: p1, checked: true);
    }
  }
  ...
  S1Interval complement() {
    ...
    return new S1Interval(hi: hi, lo: lo, checked: true); // Handles
    // empty and
    // full.
  }
   ...
   */
  bool contains1(final S1Interval y) {
   ...
   */
  bool interiorContains1(final S1Interval y) {
    if (isInverted()) {
      if (!y.isInverted()) {
        return y.lo > lo || y.hi < hi;
      }
   ...
   */
  bool intersects(final S1Interval y) {
   ...
   * intervals.
   */
  bool interiorIntersects(final S1Interval y) {
  ...
  S1Interval addPoint(double p) {
    ...
    if (isEmpty()) {
      ...
      if (dlo < dhi) {
        return new S1Interval(lo: p, hi: hi);
      } else {
        return new S1Interval(lo: lo, hi: p);
      }
  ...
  S1Interval expanded(double radius) {
    ...
    if (getLength() + 2 * radius >= 2 * PI - 1e-15) {
      return full();
    }

    // NOTE(dbeaumont): Should this remainder be 2 * PI or just PI ??
    double lo2 = (lo - radius).remainder(2 * PI);
    double hi2 = (hi + radius).remainder(2 * PI);
    if (lo2 == -PI) {
      lo2 = PI;
    }
    return new S1Interval(lo: lo2, hi: hi2);
  }
   ...
   * interval "y".
   */
  S1Interval union(final S1Interval y) {
    ...
    if (fastContains(y.lo)) {
      ...
      return new S1Interval(lo: lo, hi: y.hi, checked: true);
    }
    if (fastContains(y.hi)) {
      return new S1Interval(lo: y.lo, hi: hi, checked: true);
    }
   ...
   * disjoint intervals.
   */
  S1Interval intersection(final S1Interval y) {
    // The y.is_full() case is handled correctly in all cases by the code
    // below, but can follow three separate code paths depending on whether
    // this interval is inverted, is non-inverted but contains Pi, or neither.
   ...
   */
  bool approxEquals(final S1Interval y, double maxError) {
    if (isEmpty()) {
    ...
    return (y.lo - lo).remainder(2 * PI).abs() +
            (y.hi - hi).remainder(2 * PI).abs() <=
        maxError;
  }

  bool approxEquals1(final S1Interval y) {
   ...
   * equivalent to (drem(b - a - PI, 2 * PI) + PI), except that
